...
use clap::Parser;
use cli::Cli;
use shared::spinner_utils::{create_spinner_with_message, stop_and_persist_spinner_with_message};
use std::io;
use std::path::Path;
use std::path::PathBuf;
use shared::payload;
use shared::uv_handler::find_manifest_file;
use shared::config;
...
fn main() -> io::Result<()> {
    ...
    let manifest_path = find_manifest_file(&payload_path);
        if !manifest_path.exists() {
            eprintln!("No manifest file found in the source directory");
            std::process::exit(1);
        }
