...
fn embed_source(source_dir: &Path, output_path: &Path, cli_uv_path: PathBuf, no_uv_embed: bool, force_uv_download: bool) -> io::Result<()> {
    ...
    payload::embed_payload(&source_paths, &manifest_path, project_config, cli_uv_path, output_path, no_uv_embed, force_uv_download)
}
...
fn main() -> io::Result<()> {
    ...
    embed_source(&payload_path, &output_path, cli.uv_path, cli.no_uv_embed, cli.force_uv_download)?;
